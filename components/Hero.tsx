...
const Hero: React.FC = () => {
  ...
  const actionsY = useTransform(scrollYProgress, [0, 1], [0, 45]);

  return (
    <section ref={heroRef} className="relative py-20 md:py-28 overflow-hidden flex flex-col items-center justify-center text-center">
      <motion.div 
      ...
      >
        <motion.div
          style={{ y: badgeY }}
          className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/5 dark:bg-primary/10 text-gray-700 dark:text-primary border-2 border-black/20 dark:border-primary/20 mb-5 transform -rotate-1"
        >
        ...
        <motion.h1
          style={{ y: titleY }}
          className="text-5xl md:text-7xl font-black mb-6 leading-tight tracking-tighter text-gray-900 dark:text-white"
        >
          <motion.span
            initial={{ y: 0 }}
            animate={{ y: [0, -8, 0] }}
        ...
        <p className="text-lg md:text-xl text-gray-700 dark:text-gray-300 max-w-2xl mx-auto mb-6 font-medium leading-relaxed">
          I'm a passionate developer seeking full-time opportunities in software engineering
        </p>
        ...
        <motion.div style={{ y: actionsY }} className="flex flex-col sm:flex-row justify-center gap-4">
          <button 
